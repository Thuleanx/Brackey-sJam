...
public static class ReverseAnimationContext
{
    ...
    private static void ReverseClip(AnimationClip clip, Animator[] animators)//List<AnimatorController> animConts
    {
		...
		if (curves.Length == 0) {
			curves = AnimationUtility.GetObjectReferenceCurveBindings(clip);
			refCurve = true;
		}
        ...
        foreach (EditorCurveBinding binding in curves)
        {
			...
			if (!refCurve)
			{
				...
				int keyCount = keyframes.Length;

				for (int i = 0; i < keyCount; i++) {
					ObjectReferenceKeyframe K = keyframes[i];
					K.time = clipLength - K.time;
					keyframes[i] = K;
				}

				// reversing keyframes array
				for (int i = 0; i < keyCount; i++) if (i < keyCount - 1 - i) {
					ObjectReferenceKeyframe K = keyframes[i];
					keyframes[i] = keyframes[keyCount - 1 - i];
					keyframes[keyCount - 1 - i] = K;
				}
